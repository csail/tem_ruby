...
# Builder class and namespace for the ISA (Instruction Set Architecture) builder.
class Isa
  ...
  # 
  # The opcode's arguments are provided as an array of hashes. Each hash
  ...
  #   * name(*arguments) -> encodes the instruction
      ...
      define_method :"#{name}" do |*args|
        ...
        fargs.each_with_index do |arg, i|
          if (arg.kind_of? Numeric) && !arg[:reladdr]
            @body += abi.send arg_encode_msgs[i], arg
          else
            link_directive = { :type => iargs[i][:type],
            ...
            @body << link_directive
            @body += abi.send(arg_encode_msgs[i], 0)[1..-1]
          end
        end        
      end
    end
